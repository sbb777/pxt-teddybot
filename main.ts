/*
Louis
"teddybot": "file:../pxt-teddybot"
Refer to maqueen about IR data
*/

/** regarding IR */
let IrPressEvent = 0
let teddybotmcb: Action
let teddybotmmycb: Action
let teddybotme = "1"
let teddybotmparam = 0
let alreadyInit = 0
...
namespace teddybot {
    ...
    //% advanced=true shim=teddybotmIR::initIR
    function initIR(pin: Pins): void {
        return
    }

    //% advanced=true shim=teddybotmIR::onPressEvent
    function onPressEvent(btn: RemoteButton, body: Action): void {
        return
    }

    //% advanced=true shim=teddybotmIR::getParam
    function getParam(): number {
        return 0
    }

    function teddybotmInit(): void {
        if (alreadyInit == 1) {
            return
        }
        initIR(Pins.P16)
    ...
    //% subcategory=IRdata
    export function IR_callbackUser(teddybotmcb: (message: number) => void) {
        teddybotmInit();
        IR_callback(() => {
            const packet = new Packeta();
            packet.mye = teddybotme;
            teddybotmparam = getParam();
            packet.myparam = teddybotmparam;
            teddybotmcb(packet.myparam);
        });
    }
    ...
    */
    ...
    //% advanced=true shim=teddybotmIRV2::irCode
    ...
    function IR_callback(a: Action): void {
        teddybotmcb = a
        IrPressEvent += 1
        onPressEvent(IrPressEvent, teddybotmcb)
    }

    //% advanced=true shim=teddybotmIRV2::readPulseIn
